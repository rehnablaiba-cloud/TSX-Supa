...
/**
 * Serialise an array of row objects to CSV (or TSV with sep='\t').
 * Handles quoting, embedded commas, and double-quote escaping per RFC 4180.
 * Previously defined inline inside MobileNav.tsx.
 */
export function toCsv(rows: Record<string, unknown>[], sep = ','): string {
  if (!rows.length) return '';
  const headers = Object.keys(rows[0]);
...
/**
 * Serialise an array of row objects to INSERT SQL statements.
 * Uses ON CONFLICT DO NOTHING — safe for re-import.
 * Previously defined inline inside MobileNav.tsx.
 */
export function toSql(table: string, rows: Record<string, unknown>[]): string {
  if (!rows.length) return `-- ${table}: no rows`;
  ...
  return rows
    .map(r =>
      ...
      `VALUES (${cols.map(c => esc(r[c])).join(', ')}) ON CONFLICT DO NOTHING;`
    )
    .join('\n');
}
