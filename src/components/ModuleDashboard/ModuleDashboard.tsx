...
const ModuleDashboard: React.FC<Props> = ({ moduleId, moduleName, onBack, onExecute }) => {
  ...
  const handleExecute = (testId: string) => {
    ...
    if (lock && lock.user_id !== user?.id) return; // blocked
    onExecute(testId);
  };
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="p-6 flex flex-col gap-6 pb-24 md:pb-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <StatCard label="Total" value={tests.length} icon="📌" />
...
const TestRow: React.FC<{
  ...
  lockedByName: string;
}> = ({ test, onExecute, isLockedByOther, isLockedByMe, lockedByName }) => {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`text-sm whitespace-nowrap shrink-0 px-4 py-2 rounded-xl font-semibold transition-all ${
          isLockedByOther
