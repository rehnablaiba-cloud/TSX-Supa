...
import {
  Hash, CheckCircle, ArrowLeft, GitBranch, ChevronDown,
  ChevronRight, Lock, Upload, AlertCircle, Info, Play,
  Eye, Pencil, CloudUpload, Database,
} from "lucide-react"
...
type Stage =
  ...
  | "rev-diff"
  | "rev-confirm"
  | "rev-deploy-r2"   // R2 written, awaiting Supabase confirm
  | "submitting"
  | "done"
  | "vis-module"
  | "vis-list"
...
const TYPE_STYLES: Record<string, { badge: string; row: string }> = {
  UNCHANGED: { badge: "bg-green-500/15 text-green-400 border-green-500/20", row: "hover:bg-green-500/5" },
  EDIT:      { badge: "bg-amber-500/15 text-amber-400 border-amber-500/20", row: "hover:bg-amber-500/5" },
  INSERT:    { badge: "bg-sky-500/15 text-sky-400 border-sky-500/20",       row: "hover:bg-sky-500/5" },
  DELETE:    { badge: "bg-red-500/15 text-red-400 border-red-500/20",       row: "hover:bg-red-500/5" },
}
...
async function r2Read(token: string, key: string): Promise<any> {
  const res = await fetch(WORKER_URL, {
    method:  "POST",
    headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json" },
  ...
  <div className="rounded-xl border border-(--border-color) overflow-hidden">
    <div className="overflow-y-auto" style={{ maxHeight: "420px" }}>
      <table className="w-full text-left border-collapse">
        ...
        <tbody>
          {items.map((item, i) => <DiffTableRow key={i} item={item} index={i} />)}
        </tbody>
      </table>
    </div>
  </div>
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} className="border-b border-(--border-color) hover:bg-sky-500/5 transition-colors">
              ...
              <td className="px-2 py-2.5 text-xs text-t-secondary max-w-[200px]"><p className="truncate">{r.action || "—"}</p></td>
              <td className="px-2 py-2.5 text-xs text-t-muted max-w-[140px]"><p className="truncate">{r.expected_result || "—"}</p></td>
            </tr>
          ))}
        </tbody>
      </table>
...
const StepOrderTable: React.FC<{ stepOrder: string[]; newStepIds: Set<string> }> = ({ stepOrder, newStepIds }) => {
    ...
    <div className="flex flex-col gap-2">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {stepOrder.slice(0, visible).map((id, i) => {
                ...
                return (
                  <tr key={id} className={`border-b border-(--border-color) transition-colors ${isNew ? "hover:bg-sky-500/5" : "hover:bg-bg-base/50"}`}>
                    ...
                    <td className="px-3 py-2.5 w-20 text-center">
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border leading-none ${
                        isNew ? "bg-sky-500/15 text-sky-400 border-sky-500/20" : "bg-bg-base text-t-muted border-(--border-color)"
                      }`}>
                        {isNew ? "NEW" : "REUSED"}
                      </span>
                    </td>
      ...
      {remaining > 0 && (
        <button
          ...
          className="w-full py-2 rounded-xl border border-(--border-color) text-xs text-t-muted hover:text-t-primary hover:border-c-brand/40 transition-colors"
        >
...
const DiffStats: React.FC<{ summary: DiffResult["summary"] }> = ({ summary }) => (
  <div className="grid grid-cols-4 gap-1.5">
    {(
      ...
      <div key={label} className={`rounded-lg border px-2 py-2 text-center bg-${color}-500/5 border-${color}-500/20`}>
...
const ImportStepsModal: React.FC<Props> = ({ onClose, onBack }) => {
  ...
  const showProgress = (message: string, percent: number) => setProgress({ show: true, message, percent })
  ...
  // ── R2 deploy state ───────────────────────────────────────────────────────
  const [r2Deployed,  setR2Deployed]  = useState(false)
  const [r2DeployErr, setR2DeployErr] = useState<string | null>(null)
  ...
  // ── NO useEffect — tests loaded on button click only ─────────────────────
  ...
  const handleBack = () => {
    switch (stage) {
      ...
      case "rev-deploy-r2":   return setStage("rev-confirm")
  ...
  // ── Load tests — button-triggered, NOT on mount ───────────────────────────
  ...
  // ── loadRevisionControl — Supabase RPC for metadata only, no steps ────────
  const loadRevisionControl = async (testSerialNo: string) => {
    ...
    try {
      ...
      const allRevs = (revRows ?? []) as Array<{
        id: string; status: string; step_order: any;
        created_at: string; notes: string | null; step_count: number
      }>
      ...
      setActiveRev(activeRaw ? { ...activeRaw, step_order: parseStepOrder(activeRaw.step_order) } : null)
      setAllRevisions(allRevs.map(r => ({
        id: r.id, status: r.status, created_at: r.created_at,
        notes: r.notes, step_count: r.step_count ?? 0,
      })))
    } catch (e: any) {
      setError(e.message)
    } finally {
    ...
    try {
      if (activeRev) {
        const { error: deactErr } = await supabase
          .from("test_revisions").update({ status: "archived" }).eq("id", activeRev.id)
        if (deactErr) throw new Error(deactErr.message)
      }
      const { error: actErr } = await supabase
        .from("test_revisions").update({ status: "active" }).eq("id", revId)
  ...
  // ── handleGenerateDiff — reads base steps from R2, NOT Supabase ───────────
  const handleGenerateDiff = async () => {
    ...
    if (!isFirst && activeRev) {
      showProgress("Reading base steps from R2…", 50)
      await sleep(50)

      try {
        const token   = await getToken()
        // Read test_steps/{activeRevId}.json from R2 — zero Supabase fetches
        const r2Steps = await r2Read(token, `test_steps/${activeRev.id}.json`)
        const rows    = Array.isArray(r2Steps) ? r2Steps as any[] : []
        const stepMap = new Map(rows.map((s: any) => [s.id, s]))
        ...
        setError(`R2 read failed: ${e.message}`)
        await hideProgress(200)
        return
      }
    } else {
  ...
  // ── handleDeployR2 — writes step_orders + test_steps to R2 ───────────────
  // Full step list = reused steps (from R2 base) + new steps (from payload)
  // sorted by the new step_order sequence.
  const handleDeployR2 = async () => {
    if (!revPayload) return
    setR2DeployErr(null)
    showProgress("Writing to R2…", 10)

    try {
      const token      = await getToken()
      const fullRevId  = `${revSelTest}-${newRevId}`
      const stepOrder  = revPayload.revision.step_order

      // step_orders/{id}.json — just the ordered ID array
      showProgress("Writing step_orders…", 25)
      await r2Write(token, `step_orders/${fullRevId}.json`, stepOrder)

      // Merge reused (from baseSteps) + new rows into one sorted array
      showProgress("Merging step rows…", 45)
      const newIds = new Set(revPayload.newSteps.map(s => s.id))
      const reused = baseSteps.filter(s => stepOrder.includes(s.id) && !newIds.has(s.id))
      const all    = [...reused, ...revPayload.newSteps]
      const idxMap = new Map(stepOrder.map((id, i) => [id, i]))
      const sorted = all.sort((a, b) => (idxMap.get((a as any).id) ?? 0) - (idxMap.get((b as any).id) ?? 0))

      // test_steps/{id}.json — full rows in order
      showProgress("Writing test_steps…", 65)
      await r2Write(token, `test_steps/${fullRevId}.json`, sorted)

      showProgress("R2 deployed", 100)
      await hideProgress(300)
      setR2Deployed(true)
      setStage("rev-deploy-r2")
    } catch (e: any) {
      setR2DeployErr(e.message)
      await hideProgress()
    }
  }

  // ── handleRevisionSubmit — Supabase insert, only after R2 confirmed ───────
  // Writes: test_revisions (1 insert) + test_steps (upsert new rows only)
  const handleRevisionSubmit = async () => {
    if (!revPayload || !r2Deployed) return
    setStage("submitting")
    setError(null)
    showProgress("Saving to Supabase…", 10)

    try {
      const fullRevId = `${revSelTest}-${newRevId}`

      showProgress("Creating revision record…", 30)
      ...
      // Upsert only NEW step rows — reused rows already exist in Supabase
      showProgress("Saving new step rows…", 65)
      ...
      setError(e.message)
      setStage("rev-deploy-r2")   // stay on R2-confirmed stage, not full reset
      setProgress({ show: false, percent: 0, message: "" })
    }
  }
  ...
  // ── Visibility (untouched — module_tests only) ────────────────────────────
  const handleVisibilityClick = async () => {
    ...
    setError(null)
    try {
      const { data, error: modErr } = await supabase.rpc("get_module_names_from_module_tests")
  ...
  const loadVisibilityList = async (moduleName: string) => {
    ...
    try {
      ...
      setVisRows(((data ?? []) as any[]).map(r => ({
        id: r.id as string, module_name: r.module_name, tests_name: r.tests_name,
        is_visible: r.is_visible,
        lockInfo: r.lock_user_id ? { user_id: r.lock_user_id, locked_by_name: r.lock_user_name } : null,
      })))
    } catch (e: any) {
      setError(e.message)
    } finally {
  ...
  const handleToggleVisibility = async (row: ModuleTestVisRow) => {
    ...
    try {
      const { error: updErr } = await supabase
        .from("module_tests").update({ is_visible: !row.is_visible }).eq("id", row.id)
    ...
    const subtitle: Record<Stage, string> = {
      ...
      "rev-confirm":   "Review & Deploy to R2",
      "rev-deploy-r2": "R2 deployed — Save to Supabase",
      submitting:      "Working…",
      done:            "Done!",
    ...
    const isFirst      = !activeRev || activeRev.step_order.length === 0
    const newStepIdSet = revPayload ? new Set(revPayload.newSteps.map(s => s.id)) : new Set<string>()
    ...
    return (
      ...
      >
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-t-muted mt-0.5 leading-snug">
                  Create, activate and manage revisions · <span className="font-mono">test_revisions</span>
                </p>
              </div>
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-t-muted mt-0.5 leading-snug">
                  Toggle execute / view-only per test in a module · <span className="font-mono">module_tests.is_visible</span>
                </p>
              </div>
              <ChevronRight size={14} className="text-t-muted shrink-0 ml-auto" />
            </button>
        ...
        {stage === "revctrl-test" && (
          <div className="flex flex-col gap-2">
            {/* Load tests button — no auto-fetch */}
            {tests.length === 0 && !testsLoading && (
              <button
                onClick={loadTests}
                className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border
                  border-(--border-color) bg-bg-card hover:bg-bg-base text-sm text-t-muted transition-colors"
              >
                <Database size={14} /> Load Tests
              </button>
            )}
            {testsLoading && (
            ...
            {revLoading ? (
              ...
              <>
                <div className="rounded-xl border border-(--border-color) bg-bg-card p-3">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-t-muted mb-2">Currently Active</p>
                  {activeRev ? (
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold font-mono bg-green-500/15 text-green-400 border border-green-500/20 px-2 py-0.5 rounded-full">
                          {activeRev.id}
                        </span>
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-t-muted mt-0.5 leading-snug">
                      Import CSV → diff → deploy to R2 → save to Supabase
                    </p>
                  </div>
                </button>
                ...
                {allRevisions.length > 0 && (
                  <div className="flex flex-col gap-1.5">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-t-muted">All Revisions</p>
                    <div className="rounded-xl border border-(--border-color) overflow-hidden">
                        ...
                        <table className="w-full text-left border-collapse">
                          ...
                          <tbody>
                            {allRevisions.map(rev => {
                              ...
                              return (
                                <tr key={rev.id} className={`border-b border-(--border-color) transition-colors ${isActive ? "bg-green-500/5" : "hover:bg-bg-base/50"}`}>
                                  ...
                                  <td className="px-3 py-2.5 text-right">
                                    {!isActive && (
                                      <button
                                        ...
                                        disabled={revLoading}
                                        className="flex items-center gap-1 text-[11px] font-medium text-c-brand hover:text-c-brand/80 transition-colors disabled:opacity-50 ml-auto"
                                      >
        ...
        {stage === "rev-info" && (
          <div className="flex flex-col gap-3">
            <div className="rounded-xl border border-(--border-color) bg-bg-card p-3">
              <p className="text-[10px] font-bold uppercase tracking-wider text-t-muted mb-2">Current Active Revision</p>
              {activeRev ? (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold font-mono bg-green-500/15 text-green-400 border border-green-500/20 px-2 py-0.5 rounded-full">
            ...
            <div className="rounded-xl border border-(--border-color) bg-bg-card p-3 flex flex-col gap-2.5">
              ...
              <div>
                <label className="block text-[10px] text-t-muted mb-1">Revision code (e.g. R0, RA-1, RB-2)</label>
                <input
                  value={newRevId}
                ...
                {existingRevIds.length > 0 && (
                  <p className="text-[10px] text-t-muted mt-1.5">Existing: {existingRevIds.join(" · ")}</p>
                )}
              </div>
            </div>
            ...
            <div className="rounded-xl border border-(--border-color) bg-bg-card p-3 flex flex-col gap-2">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg border border-dashed border-(--border-color) text-xs text-t-muted hover:text-t-primary hover:border-c-brand/40 transition-colors"
              >
            ...
            {parseResult && parseResult.errors.length > 0 && (
              <div className="flex flex-col gap-1">
                {parseResult.errors.map((e, i) => (
                  <div key={i} className="flex items-start gap-2 text-xs text-red-400 bg-red-500/8 border border-red-500/20 rounded-lg px-3 py-2">
        ...
        {stage === "rev-diff" && parseResult && (
          <div className="flex flex-col gap-3">
            {isFirst ? (
              <div className="text-xs font-semibold text-green-400 bg-green-500/8 border border-green-500/20 rounded-lg px-3 py-2.5">
                ✦ First import — {parseResult.rows.length} steps →&nbsp;
                <span className="font-mono">{newRevId}</span>. All rows become new step records.
              </div>
            ) : diffResult ? (
              <>
                <div className="text-xs font-semibold text-c-brand bg-c-brand/8 border border-c-brand/20 rounded-lg px-3 py-2.5">
            ...
            {isFirst ? <FirstImportTable rows={parseResult.rows} /> : diffResult ? <DiffTable items={diffResult.items} /> : null}
  
            {parseResult.warnings.length > 0 && (
              <div className="flex flex-col gap-1">
                {parseResult.warnings.map((w, i) => (
                  <div key={i} className="text-[11px] text-amber-400 bg-amber-500/8 border border-amber-500/20 rounded-lg px-3 py-1.5">⚠ {w}</div>
                ))}
              </div>
            )}
  
            <button onClick={handleBuildPayload} disabled={progress.show} className="btn-primary text-sm disabled:opacity-50">
              Continue →
            </button>
          </div>
        )}
        ...
        {stage === "rev-confirm" && revPayload && (
          <div className="flex flex-col gap-3">
            ...
            <div>
              ...
              <StepOrderTable stepOrder={revPayload.revision.step_order} newStepIds={newStepIdSet} />
            </div>
            ...
            {r2DeployErr && (
              <div className="flex items-start gap-2 px-3 py-2 rounded-xl bg-red-500/10 border border-red-500/20">
                ...
                <p className="text-xs text-red-400">{r2DeployErr}</p>
              </div>
            )}
        ...
        {/* ══ rev-deploy-r2 ══ */}
        {stage === "rev-deploy-r2" && revPayload && (
          <div className="flex flex-col gap-3">
            {/* R2 success banner */}
            <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-green-500/20 bg-green-500/5">
              <CheckCircle size={15} className="text-green-400 shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-green-400">R2 deployed</p>
                <p className="text-[11px] text-t-muted truncate">
                  step_orders/{revSelTest}-{newRevId}.json · test_steps/{revSelTest}-{newRevId}.json
                </p>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-(--border-color) bg-bg-card p-3 flex flex-col gap-1.5 text-xs">
              ...
              <Row label="Steps in order" value={String(revPayload.revision.step_order.length)} mono />
              <Row label="New step rows"  value={String(revPayload.newSteps.length)} mono />
              <Row label="Reused rows"    value={String(revPayload.revision.step_order.length - revPayload.newSteps.length)} mono />
            </div>
              ...
              <div className="flex flex-col gap-1.5 max-h-60 overflow-y-auto">
                {visModules.map(name => (
                  <button
                    ...
                    className="text-left px-3 py-2.5 rounded-xl border border-(--border-color) bg-bg-card hover:bg-bg-base text-sm text-t-primary transition-colors"
                  >
                    {name}
                  </button>
                ))}
        ...
        {stage === "vis-list" && (
          <div className="flex flex-col gap-3">
            {visLoading ? (
              ...
              <p className="text-sm text-t-muted text-center py-8">No tests found in this module.</p>
            ) : (
              <>
                      ...
                      <tbody>
                        {visRows.map(row => (
                          <tr key={row.id} className="border-b border-(--border-color) hover:bg-bg-base/50 transition-colors">
                            <td className="px-3 py-2.5 text-sm text-t-primary font-medium">{row.tests_name}</td>
                            <td className="px-3 py-2.5 w-28 text-center">
                              {row.lockInfo ? (
                                <div
                                  title={`Locked by ${row.lockInfo.locked_by_name} — finish test to unlock`}
                                  className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-lg border text-amber-400 border-amber-500/30 bg-amber-500/8 cursor-not-allowed select-none"
                                >
                                  <Lock size={11} />
                                ...
                                <button
                                  ...
                                  disabled={visLoading}
                                  title={row.is_visible ? "Execute mode — click to make view-only" : "View-only — click to enable execution"}
                                  className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-lg border transition-colors disabled:opacity-50 ${
                                    row.is_visible
                                ...
                                >
                                  {row.is_visible ? <><Pencil size={11} /> Execute</> : <><Eye size={11} /> View Only</>}
                                </button>
                              )}
                            </td>
                          </tr>
