  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      if (format === "json") {
        downloadBlob(
          new Blob(
            [JSON.stringify({ exported_at: new Date().toISOString(), tables: allData }, null, 2)],
        ...
        downloadBlob(new Blob([lines.join("\n")], { type: "text/plain" }), `testpro-full-${today}.sql`);
      } else {
                ...
                {counts.map(({ table, count }) => (
                  <div key={table} className="flex items-center justify-between px-3 py-2.5">
                    ...
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                      count > 0 ? "bg-c-brand-bg text-c-brand" : "bg-bg-card text-t-muted"
                    }`}>
                      {count} rows
                    </span>
                  </div>
