...
// Light revision — NO step_order, NO heavy columns
...
// Light revision list — no step_order, no heavy columns, optional status filter
async function fetchRevisionList(status?: string): Promise<TestRevision[]> {
  let q = supabase
    .from("test_revisions")
    .select("id, revision, tests_serial_no, status")  // ← NO step_order
    .order("tests_serial_no")
  if (status) q = q.eq("status", status)
  const { data, error } = await q
  if (error) throw new Error(error.message)
  return (data ?? []) as TestRevision[]
}

// Fetch step_order for ONE revision by ID — single row, one column
async function fetchStepOrder(revisionId: string): Promise<string[]> {
  const { data, error } = await supabase
    ...
    .single()
  if (error) throw new Error(error.message)
  return (data?.step_order as string[]) ?? []
}

// Fetch test_steps by exact IDs from step_order (PK .in() — no serial_no)
async function fetchStepsByIds(ids: string[]): Promise<any[]> {
  if (ids.length === 0) return []
  const { data, error } = await supabase
    .from("test_steps")
    .select("*")
    .in("id", ids)           // ← exact IDs only, never serial_no
  if (error) throw new Error(error.message)
  // Re-sort to match step_order sequence (.in does not guarantee order)
  const idxMap = new Map(ids.map((id, i) => [id, i]))
  return (data ?? []).sort((a: any, b: any) => (idxMap.get(a.id) ?? 0) - (idxMap.get(b.id) ?? 0))
}

// RPC A — ordered steps via unnest join (1 round-trip, server-sorted)
async function fetchOrderedStepsRpc(revisionId: string): Promise<any[]> {
  const { data, error } = await supabase
...
// RPC B — count how many of the expected step IDs actually exist in DB
async function countStepsByIds(ids: string[]): Promise<number> {
  if (ids.length === 0) return 0
  const { data, error } = await supabase
    .rpc("count_steps_by_ids", { p_ids: ids })
  if (error) throw new Error(error.message)
...
function ValidationCard({
  ...
  const statusConfig: Record<SyncStatus, { badge: string; badgeBg: string; label: string }> = {
    unknown:        { badge: "text-t-muted",    badgeBg: "bg-bg-card",       label: "Not checked" },
    checking_r2:    { badge: "text-c-brand",    badgeBg: "bg-c-brand/10",    label: "Fetching R2…" },
    checking_db:    { badge: "text-c-brand",    badgeBg: "bg-c-brand/10",    label: "Querying DB…" },
    in_sync:        { badge: "text-green-400",  badgeBg: "bg-green-500/10",  label: "In sync" },
    count_mismatch: { badge: "text-yellow-400", badgeBg: "bg-yellow-500/10", label: "Count drift" },
    id_mismatch:    { badge: "text-orange-400", badgeBg: "bg-orange-500/10", label: "ID mismatch" },
    missing_r2:     { badge: "text-red-400",    badgeBg: "bg-red-500/10",    label: "Missing in R2" },
    error:          { badge: "text-red-400",    badgeBg: "bg-red-500/10",    label: "Error" },
  }
        ...
        <div className="flex items-center gap-1 text-[10px] font-mono">
          {entry.r2Count !== null ? (
            ...
            <span className="px-1.5 py-0.5 rounded-md bg-c-brand/10 text-c-brand border border-c-brand/20 flex items-center gap-1">
              <div className="w-2 h-2 border border-c-brand border-t-transparent rounded-full animate-spin" /> R2
            </span>
          ) : null}
          ...
          {entry.dbCount !== null ? (
            ...
            <span className="px-1.5 py-0.5 rounded-md bg-bg-base text-t-muted border-(--border-color) flex items-center gap-1">
              <div className="w-2 h-2 border border-t-muted border-t-transparent rounded-full animate-spin" /> DB
            </span>
          ) : null}
        </div>
        ...
        <div className="px-3 pb-2.5 flex flex-wrap items-center gap-2 border-t border-(--border-color)/50">
          ...
          {(isDrifted || isMissing) && (
            <button onClick={onSyncNow}
              ...
              <CloudUpload size={10} /> Upload to sync <ArrowRight size={9} />
            </button>
          )}
        </div>
      )}
...
const TYPE_META: {
  ...
  { id: "validate",           label: "Validate",           icon: <ShieldCheck size={18} />, desc: "Compare R2 snapshots vs live Supabase — tiered, zero-waste checks",                    flow: "validate",           group: "sync" },
  { id: "validate_revisions", label: "Validate Steps",     icon: <ShieldCheck size={18} />, desc: "Per-revision step_orders & test_steps R2 vs DB — button-triggered, chunked diff",    flow: "validate_revisions", group: "sync" },
  { id: "modules",            label: "Modules",            icon: <Package size={18} />,     desc: "Upload all modules → modules/all.json",                                               flow: "direct",             group: "sync" },
  { id: "tests",              label: "Tests",              icon: <FlaskConical size={18} />,desc: "Upload all tests → tests/all.json",                                                   flow: "direct",             group: "sync" },
  { id: "revisions",         label: "Revisions",           icon: <GitBranch size={18} />,   desc: "Upload all test revisions → revisions/all.json",                                     flow: "direct",             group: "sync" },
  { id: "step_orders",       label: "Step Orders",         icon: <ListOrdered size={18} />, desc: "Upload step_order per revision → step_orders/{id}.json",                             flow: "revision",           group: "manage" },
  { id: "test_steps",        label: "Test Steps",          icon: <Layers size={18} />,      desc: "Upload test_steps per revision → test_steps/{id}.json",                              flow: "revision",           group: "manage" },
  { id: "delete_step_orders",label: "Delete Step Orders",  icon: <Trash2 size={18} />,      desc: "Delete step_orders/{id}.json files from R2",     flow: "delete_revision", group: "util", danger: true },
  { id: "delete_test_steps", label: "Delete Test Steps",   icon: <Trash2 size={18} />,      desc: "Delete test_steps/{id}.json files from R2",      flow: "delete_revision", group: "util", danger: true },
  { id: "reader",            label: "Read from R2",        icon: <Eye size={18} />,         desc: "Fetch any R2 key and inspect its JSON",                                              flow: "reader",             group: "util" },
]
...
const R2MigrationModal: React.FC<Props> = ({ onClose, onBack }) => {
  ...
  // ── Load revision list (button-triggered, NOT useEffect) ───────────────────
  ...
  const loadRevisionsForValidate = async () => {
    setRevLoading(true)
  ...
  const handleTypeSelect = (t: MigrationType) => {
    setMigrType(t)
    setError(null)
    setRevisions([])       // always clear stale list — user must click Load
  ...
  // ── Revision upload ────────────────────────────────────────────────────────
  // step_orders: fetchStepOrder (1 row, 1 col) → write directly — no test_steps query
  // test_steps:  fetchOrderedStepsRpc (RPC A) — 1 round-trip, server-sorted, no serial_no
    ...
    for (const rev of chosenRevs) {
      ...
      try {
        if (migrType === "step_orders") {
          // 1 row, 1 column — PK filter, no serial_no
          const order = await fetchStepOrder(rev.id)
          if (order.length === 0) throw new Error("step_order is empty for this revision")
          ...
          // RPC A: get_ordered_steps — server-sorted, 1 round-trip, no serial_no
          const steps = await fetchOrderedStepsRpc(rev.id)
          await r2Write(token, `test_steps/${rev.id}.json`, steps)
          patch(rowLabel, { status: "done", count: steps.length })
          done.push({ label: rowLabel, count: steps.length })
        }
      } catch (e: any) {
  ...
  // ── 3-Tier Validation (modules / tests / revisions bulk) ──────────────────
  const handleValidate = async () => {
    setStage("validating")
    setValidationError(null)
    const entries: ValidationEntry[] = VALIDATION_TARGETS.map(t => ({
      ...t, r2Data: null, r2Count: null, r2Ids: null,
      dbCount: null, dbIds: null, diff: null, tier2: "idle", status: "checking_r2",
    }))
    ...
    await Promise.all(entries.map(async (_, i) => {
      try {
        ...
        entries[i] = { ...entries[i], r2Data: arr, r2Count: arr.length,
          r2Ids: new Set(arr.map(r => String(r[entries[i].pkField]))), status: "checking_db" }
      } catch {
  ...
  const handleTier2 = async (idx: number) => {
    ...
    try {
      ...
      patchEntry(idx, { tier2: "done", diff: {
        added:    [...dbMap.keys()].filter(k => !r2Map.has(k)),
        removed:  [...r2Map.keys()].filter(k => !dbMap.has(k)),
        modified: [...r2Map.keys()].filter(k => dbMap.has(k) && r2Map.get(k) !== dbMap.get(k)),
      }})
    } catch (e: any) {
  ...
  const handleSyncFromValidation = (entry: ValidationEntry) => {
    const map: Partial<Record<string, MigrationType>> = {
      "modules/all.json": "modules", "tests/all.json": "tests", "revisions/all.json": "revisions",
    }
  ...
  // SO: fetchStepOrder (1 row, 1 col) vs R2 array — no serial_no
  // TS: countStepsByIds RPC B (1 bigint) → chunkedDeepDiff .in("id", chunk) — no serial_no
  const handleValidateRevisions = async () => {
    ...
    for (let i = 0; i < chosenRevs.length; i++) {
      ...
      // ── step_order validation ───────────────────────────────────────────────
      items[i] = { ...items[i], soStatus: "running" }
      setRevValidation([...items])
      try {
        const soR2: string[] = await r2Read(token, `step_orders/${rev.id}.json`)
          .then(d => Array.isArray(d) ? d : [])
          .catch((e: any) => { throw new Error(e.message) })

        // 1 row, 1 column — PK filter — never serial_no
        const soDb = await fetchStepOrder(rev.id)
  ...
  const handleRead = async () => {
    ...
    setReaderLoading(true); setReaderError(null); setReaderData(null)
    try {
      const token = await getToken()
      setReaderData(await r2Read(token, readerKey.trim()))
    } catch (e: any) {
  ...
  const handleBack = () => {
    ...
    if (stage === "validating" || stage === "validation_done") { setStage("selecttype"); setValidationEntries([]); setValidationError(null) }
  ...
  const toggleOne    = (rev: TestRevision) => {
    const next = new Set(selected)
    if (next.has(rev.id)) { next.delete(rev.id) } else {
      next.add(rev.id)
      if (!r2Status.has(rev.id) || r2Status.get(rev.id) === "unknown") checkOne(rev)
    }
    setSelected(next)
  }
  ...
  const rvAllGood = rvTotal > 0 && (rvTotal - rvSoOk) === 0 && (rvTotal - rvTsOk) === 0
    ...
    >
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {utilTypes.map(m => (
              <button key={m.id} onClick={() => handleTypeSelect(m.id)}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-all
                  ${m.danger ? "border-red-500/20 bg-red-500/5 hover:bg-red-500/10" : "border-(--border-color) bg-bg-card hover:bg-bg-base"}`}>
      ...
      {stage === "selectrevisions" && (
        <div className="flex flex-col gap-3">
          <p className="text-xs text-t-muted">
            {isDeleteFlow ? "Select revisions to delete from R2"
              : migrType === "step_orders" ? "Select revisions to upload their step_order"
              : "Select revisions to upload their test_steps"}
          </p>

          {/* Load button — NOT auto-fetched */}
            ...
            <>
              ...
              <div className="flex flex-wrap items-center gap-1.5 p-2.5 rounded-xl border border-(--border-color) bg-bg-card">
                ...
                <button onClick={checkAll} disabled={checkingAll}
                  ...
                  <RefreshCw size={10} className={checkingAll ? "animate-spin" : ""} /> Check all R2
                </button>
                {missingCount > 0 && (
                  <button onClick={selectMissing}
                    className="flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded-lg
                ...
                {checkedCount > 0 && <span className="ml-auto text-[10px] text-t-muted">{existsCount} in R2 · {missingCount} missing</span>}
              </div>

              <div className="flex items-center justify-between">
                <button onClick={toggleAll}
                  className="flex items-center gap-2 text-xs text-t-muted hover:text-t-primary transition-colors px-1">
                  {allSelected ? <CheckSquare size={14} className="text-c-brand" />
                    : noneSelected ? <Square size={14} />
              ...
              <div className="flex flex-col gap-1 max-h-64 overflow-y-auto">
                {revisions.map(r => {
                  ...
                  return (
                    <button key={r.id} onClick={() => toggleOne(r)}
                      className={`flex items-center gap-2.5 px-3 py-2 rounded-xl border text-left text-sm transition-all
                        ${selected.has(r.id)
                          ? isDeleteFlow ? "border-red-500/30 bg-red-500/5 text-t-primary" : "border-c-brand/40 bg-c-brand/5 text-t-primary"
              ...
              {isDeleteFlow ? (
                <button onClick={handleRevisionDelete} disabled={selected.size === 0}
                  className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold
                    bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20
                    disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                  <Trash2 size={14} /> Delete {selected.size} file{selected.size !== 1 ? "s" : ""} from R2
                </button>
              ) : (
                <button onClick={handleRevisionUpload} disabled={selected.size === 0}
                  className="btn-primary text-sm flex items-center justify-center gap-2 disabled:opacity-50">
                  <CloudUpload size={14} /> Upload {selected.size} revision{selected.size !== 1 ? "s" : ""}
                </button>
              )}
            </>
          )}
      ...
      {stage === "selectrevisions_validate" && (
        <div className="flex flex-col gap-3">
          ...
          {/* Load button — NOT auto-fetched */}
          {revisions.length === 0 && !revLoading && (
            <button onClick={loadRevisionsForValidate}
              className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-(--border-color) bg-bg-card hover:bg-bg-base text-sm text-t-muted transition-colors">
              <Search size={13} /> Load Revisions
            </button>
          )}
          ...
          {!revLoading && revisions.length > 0 && (
            <>
              <div className="flex items-center justify-between px-1">
                <button
                  onClick={() => setRevValSelected(
                    revValSelected.size === revisions.length ? new Set() : new Set(revisions.map(r => r.id))
                  )}
                  className="flex items-center gap-2 text-xs text-t-muted hover:text-t-primary transition-colors">
                  {revValSelected.size === revisions.length ? <CheckSquare size={14} className="text-c-brand" />
                    : revValSelected.size === 0 ? <Square size={14} /> : <MinusSquare size={14} />}
              ...
              <button onClick={handleValidateRevisions} disabled={revValSelected.size === 0}
                className="btn-primary text-sm flex items-center justify-center gap-2 disabled:opacity-50">
                <ShieldCheck size={14} /> Validate {revValSelected.size} revision{revValSelected.size !== 1 ? "s" : ""}
              </button>
            </>
          )}
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            {validationEntries.map((entry, i) => (
              <ValidationCard key={entry.key} entry={entry}
      ...
      {stage === "validation_done" && (
        <div className="flex flex-col gap-3">
          <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border
            ${driftCount > 0 ? "border-yellow-500/20 bg-yellow-500/5" : "border-green-500/20 bg-green-500/5"}`}>
            {driftCount > 0
            ...
            <div className="flex-1">
              <p className={`text-sm font-semibold ${driftCount > 0 ? "text-yellow-400" : "text-green-400"}`}>
                {driftCount > 0 ? `${driftCount} source${driftCount > 1 ? "s" : ""} out of sync` : "All sources in sync"}
              </p>
              <p className="text-xs text-t-muted">
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-1 px-1">
            {[
              ...
              <span key={l.label} className={`flex items-center gap-1 text-[9px] ${l.color}`}>{l.icon} {l.label}</span>
            ))}
          </div>
      ...
      {/* ── Validating / Revision validation done ── */}
      {(stage === "validating_revisions" || stage === "revision_validation_done") && (
        <div className="flex flex-col gap-3">
          {stage === "validating_revisions" && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-c-brand/5 border border-c-brand/15">
              ...
              <p className="text-[11px] text-t-muted">
                ...
                <span className="font-semibold text-c-brand">SO</span> = step_order (1 row, 1 col) ·{" "}
                <span className="font-semibold text-c-brand">TS</span> = RPC count + .in() chunks ({TIER2_CHUNK_SIZE}/page) · fail-fast
              </p>
            </div>
          )}
          ...
          {stage === "revision_validation_done" && (
            <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border
              ${rvAllGood ? "border-green-500/20 bg-green-500/5" : "border-yellow-500/20 bg-yellow-500/5"}`}>
              {rvAllGood
                ? <CheckCircle   size={15} className="text-green-400 shrink-0" />
                : <AlertTriangle size={15} className="text-yellow-400 shrink-0" />}
              <div className="flex-1">
                <p className={`text-sm font-semibold ${rvAllGood ? "text-green-400" : "text-yellow-400"}`}>
                  {rvAllGood ? "All revisions in sync" : `SO: ${rvTotal - rvSoOk} issue${rvTotal - rvSoOk !== 1 ? "s" : ""} · TS: ${rvTotal - rvTsOk} issue${rvTotal - rvTsOk !== 1 ? "s" : ""}`}
                </p>
                <p className="text-xs text-t-muted">{rvTotal} revisions · {rvSoOk} SO ok · {rvTsOk} TS ok</p>
              </div>
              <button onClick={() => setStage("selectrevisions_validate")}
                className="flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded-lg
                  bg-bg-base hover:bg-bg-card text-t-muted border border-(--border-color) transition-colors shrink-0">
          ...
          <div className="flex flex-col gap-1 max-h-96 overflow-y-auto">
            {revValidation.map(item => <RevisionValidationRow key={item.revId} item={item} />)}
          </div>

          {stage === "revision_validation_done" && (
            <div className="flex gap-2">
              <button onClick={() => { setStage("selecttype"); setRevValidation([]); setRevValSelected(new Set()) }} className="flex-1 btn-ghost text-sm">Back</button>
              <button onClick={onClose} className="flex-1 btn-primary text-sm">Close</button>
            </div>
          )}
        </div>
      ...
      {stage === "reader" && (
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            <input value={readerKey} onChange={e => setReaderKey(e.target.value)}
          ...
          {readerData !== null && (
            <div className="flex flex-col gap-1">
              ...
              <pre className={`text-xs bg-bg-card border border-(--border-color) rounded-xl p-3 overflow-auto font-mono text-t-primary ${showFull ? "max-h-[60vh]" : "max-h-48"}`}>
