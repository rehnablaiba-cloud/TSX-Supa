...
const FORMAT_META: { id: ExportFormat; label: string; icon: React.ReactNode; desc: string }[] = [
  { id: 'csvzip', label: 'CSV (zip)',  icon: <BarChart2 size={20} />, desc: 'One CSV per table — re-importable' },
  { id: 'json',   label: 'JSON',       icon: <FileJson size={20} />,  desc: 'All tables in one nested file'    },
  { id: 'tsvzip', label: 'TSV (zip)',  icon: <Table2 size={20} />,    desc: 'Tab-separated — Excel-friendly'   },
...
const ExportDataModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      if (format === 'json') {
        downloadBlob(
          new Blob([JSON.stringify({ exportedat: new Date().toISOString(), tables: allData }, null, 2)], { type: 'application/json' }),
...
')], { type: 'text/plain' }), `testpro-full-${stamp}.sql`);
  ...
  return (
    <ModalShell title="Export All Data" icon={<Upload size={16} />} onClose={onClose}
      subtitle={
...
type ModuleManualStage = 'selectop' | 'selectmodule' | 'fillform' | 'confirm' | 'submitting' | 'done';

const MODULE_OP_META = [
  { id: 'create' as ModuleOp, label: 'Create', icon: <Plus size={20} />,  desc: 'Add a new module'    },
  { id: 'update' as ModuleOp, label: 'Update', icon: <Pencil size={20} />, desc: 'Edit module details' },
  { id: 'delete' as ModuleOp, label: 'Delete', icon: <Trash2 size={20} />, desc: 'Remove a module'     },
];

const ImportModulesModal: React.FC<{ onClose: () => void; onDone: () => void }> = ({ onClose, onDone }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (op === 'create') {
        const { error: e } = await supabase.from('modules').insert({ name: name.trim(), description: desc.trim() || null });
        if (e) throw new Error(e.message);
      } else if (op === 'update' && selectedModule) {
        const { error: e } = await supabase.from('modules').update({ name: name.trim(), description: desc.trim() || null }).eq('name', selectedModule.name);
        if (e) throw new Error(e.message);
      } else if (op === 'delete' && selectedModule) {
        const { error: e } = await supabase.from('modules').delete().eq('name', selectedModule.name);
        if (e) throw new Error(e.message);
      }
  ...
  const subtitle = stage === 'selectop' ? 'Choose operation' : stage === 'selectmodule' ? 'Pick a module' : stage === 'fillform' ? 'Enter details' : stage === 'confirm' ? 'Review & confirm' : stage === 'done' ? 'Done!' : '…';

  return (
    <ModalShell title="Modules" icon={<Package size={16} />} subtitle={subtitle} onClose={onClose}>
      {stage === 'selectop' && (
        <div className="flex flex-col gap-2">
          {MODULE_OP_META.map(m => (
            <button key={m.id} onClick={() => handleOpSelect(m.id)}
              ...
              {m.icon}
              <div><p className="text-sm font-semibold text-t-primary">{m.label}</p><p className="text-xs text-t-muted">{m.desc}</p></div>
            </button>
          ))}
        </div>
      )}
      ...
      {stage === 'fillform' && (
        <div className="flex flex-col gap-3">
          <div>
            ...
            <input value={name} onChange={e => setName(e.target.value)} className="input text-sm" placeholder="e.g. CAR-01" />
          </div>
          <div>
            <label className="block text-xs text-t-muted mb-1">Description (optional)</label>
            <input value={desc} onChange={e => setDesc(e.target.value)} className="input text-sm" placeholder="Short description" />
          </div>
          <button onClick={() => setStage('confirm')} disabled={!name.trim()}
            className="btn-primary text-sm disabled:opacity-50">Review</button>
        </div>
      )}
      {stage === 'confirm' && (
        <div className="flex flex-col gap-3">
          <div className="rounded-xl border border-[var(--border-color)] bg-bg-card p-3 flex flex-col gap-1.5 text-xs">
            ...
            {op !== 'delete' && <><Row label="Name" value={name} /><Row label="Desc" value={desc || '—'} /></>}
          </div>
          {error && <p className="text-xs text-red-400">{error}</p>}
          <div className="flex gap-2">
            <button onClick={() => setStage(op === 'create' ? 'fillform' : op === 'delete' ? 'selectmodule' : 'fillform')}
              className="flex-1 px-4 py-2.5 rounded-xl border border-[var(--border-color)] text-t-secondary text-sm">Back</button>
            <button onClick={handleSubmit}
              className={`flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold text-white ${op === 'delete' ? 'bg-red-500 hover:bg-red-600' : 'btn-primary'}`}>
              Confirm {op}
            </button>
          </div>
        </div>
...
type TestManualStage = 'selectop' | 'selecttest' | 'fillform' | 'confirm' | 'submitting' | 'done';

const TEST_OP_META = [
  { id: 'create' as TestOp, label: 'Create', icon: <Plus size={20} />,  desc: 'Add a new test'    },
  { id: 'update' as TestOp, label: 'Update', icon: <Pencil size={20} />, desc: 'Edit test details' },
  { id: 'delete' as TestOp, label: 'Delete', icon: <Trash2 size={20} />, desc: 'Remove a test'     },
];

const ImportTestsModal: React.FC<{ onClose: () => void; onDone: () => void }> = ({ onClose, onDone }) => {
  ...
  const handleOpSelect = (o: TestOp) => {
    setOp(o); setStage(o === 'create' ? 'fillform' : 'selecttest');
  };
  ...
  return (
    <ModalShell title="Tests" icon={<FlaskConical size={16} />} subtitle={
      stage === 'selectop' ? 'Choose operation' : stage === 'selecttest' ? 'Pick a test' :
      stage === 'fillform' ? 'Enter details' : stage === 'confirm' ? 'Review & confirm' : '…'
    } onClose={onClose}>
      {stage === 'selectop' && (
        <div className="flex flex-col gap-2">
          {TEST_OP_META.map(m => (
            <button key={m.id} onClick={() => handleOpSelect(m.id)}
              className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[var(--border-color)] bg-bg-card hover:bg-bg-base text-left transition-all">
              {m.icon}<div><p className="text-sm font-semibold text-t-primary">{m.label}</p><p className="text-xs text-t-muted">{m.desc}</p></div>
            </button>
          ))}
        </div>
      )}
      ...
      {stage === 'fillform' && (
        <div className="flex flex-col gap-3">
          <div>
            ...
            <input value={sn} onChange={e => setSn(e.target.value)} className="input text-sm" placeholder="e.g. 1.1" type="number" step="0.01" />
          </div>
          <div>
            <label className="block text-xs text-t-muted mb-1">Test Name</label>
            <input value={name} onChange={e => setName(e.target.value)} className="input text-sm" placeholder="e.g. Pantograph Test" />
          </div>
      ...
      {stage === 'confirm' && (
        <div className="flex flex-col gap-3">
          <div className="rounded-xl border border-[var(--border-color)] bg-bg-card p-3 flex flex-col gap-1.5 text-xs">
            ...
            {op === 'create' && <><Row label="S/N" value={sn} mono /><Row label="Name" value={name} /></>}
          </div>
          {error && <p className="text-xs text-red-400">{error}</p>}
          <div className="flex gap-2">
            <button onClick={() => setStage(op === 'create' ? 'fillform' : 'selecttest')}
              className="flex-1 px-4 py-2.5 rounded-xl border border-[var(--border-color)] text-t-secondary text-sm">Back</button>
            <button onClick={handleSubmit}
              className={`flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold text-white ${op === 'delete' ? 'bg-red-500 hover:bg-red-600' : 'btn-primary'}`}>
              Confirm {op}
      ...
      {stage === 'submitting' && <div className="flex items-center justify-center py-8"><div className="w-8 h-8 border-4 border-c-brand border-t-transparent rounded-full animate-spin" /></div>}
      {stage === 'done' && (
        <div className="flex flex-col items-center gap-3 py-6">
          <CheckCircle size={32} className="text-green-400" />
...
// ══════════════════════════════════════════════════════════════════════════════
type StepCsvStage = 'selectmodule' | 'selecttest' | 'upload' | 'preview' | 'confirm' | 'submitting' | 'done' | 'error';

const ImportStepsModal: React.FC<{ onClose: () => void; onDone: () => void }> = ({ onClose, onDone }) => {
  ...
  const handleModuleSelect = async (mod: string) => {
    ...
    setTests(rows); setStage('selecttest');
  };
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = ev => {
      ...
      setParsed(rows); setParseErrors(errors);
  ...
  return (
    <ModalShell title="Import Steps (CSV)" icon={<Hash size={16} />} subtitle={
      stage === 'selectmodule' ? 'Pick a trainset' : stage === 'selecttest' ? 'Pick a test' :
      stage === 'upload' ? 'Upload CSV' : stage === 'preview' ? `${parsed.length} steps parsed` :
      stage === 'confirm' ? 'Review & confirm' : stage === 'done' ? 'Done!' : '…'
    } onClose={onClose}>
      {stage === 'selectmodule' && (
        <div className="flex flex-col gap-1.5 max-h-60 overflow-y-auto">
          {modules.map(m => (
            <button key={m.name} onClick={() => handleModuleSelect(m.name)}
              className="text-left px-3 py-2 rounded-xl border border-[var(--border-color)] bg-bg-card hover:bg-bg-base text-sm text-t-primary">{m.name}</button>
          ))}
        </div>
      )}
      {stage === 'selecttest' && (
        <div className="flex flex-col gap-1.5 max-h-60 overflow-y-auto">
          {tests.map(t => (
            <button key={t.id} onClick={() => { setSelTest(t.testsname); setStage('upload'); }}
              className="text-left px-3 py-2 rounded-xl border border-[var(--border-color)] bg-bg-card hover:bg-bg-base text-sm text-t-primary">{t.testsname}</button>
          ))}
        </div>
      )}
      {stage === 'upload' && (
      ...
      {(stage === 'preview' || stage === 'confirm') && (
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => setStage('upload')} className="flex-1 px-4 py-2.5 rounded-xl border border-[var(--border-color)] text-t-secondary text-sm">Back</button>
            <button onClick={handleSubmit} className="flex-1 btn-primary text-sm">Upsert {parsed.length} steps</button>
          </div>
        </div>
      )}
      {stage === 'submitting' && <div className="flex items-center justify-center py-8"><div className="w-8 h-8 border-4 border-c-brand border-t-transparent rounded-full animate-spin" /></div>}
...
const ImportStepsManualModal: React.FC<{ onClose: () => void; onDone: () => void }> = ({ onClose, onDone }) => {
  ...
  return (
    <ModalShell title="Steps (Manual)" icon={<Hash size={16} />} subtitle={
      stage === 'selectop' ? 'Choose operation' : stage === 'selectmodule' ? 'Pick trainset' :
      stage === 'selecttest' ? 'Pick test' : stage === 'selectstep' ? 'Pick step' :
      stage === 'fillform' ? 'Enter details' : stage === 'confirm' ? 'Review & confirm' : '…'
    } onClose={onClose}>
      {stage === 'selectop' && (
        <div className="flex flex-col gap-2">
          {(['create','update','delete'] as StepOp[]).map(o => (
            <button key={o} onClick={() => { setOp(o); setStage('selectmodule'); }}
              className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[var(--border-color)] bg-bg-card hover:bg-bg-base text-left transition-all">
              {o === 'create' ? <Plus size={20} /> : o === 'update' ? <Pencil size={20} /> : <Trash2 size={20} />}
      ...
      {stage === 'selecttest' && (
        <div className="flex flex-col gap-1.5 max-h-60 overflow-y-auto">
          {tests.map(t => (
            <button key={t.id} onClick={() => handleTestSelect(t.testsname)}
              className="text-left px-3 py-2 rounded-xl border border-[var(--border-color)] bg-bg-card hover:bg-bg-base text-sm text-t-primary">{t.testsname}</button>
          ))}
        </div>
      )}
      ...
      {stage === 'fillform' && (
        <div className="flex flex-col gap-3">
          <div><label className="block text-xs text-t-muted mb-1">Serial No</label>
            <input value={sn} onChange={e => setSn(e.target.value)} className="input text-sm" type="number" step="0.01" /></div>
          <div><label className="block text-xs text-t-muted mb-1">Action</label>
            <textarea value={action} onChange={e => setAction(e.target.value)} className="input text-sm resize-none" rows={3} /></div>
      ...
      {stage === 'confirm' && (
        <div className="flex flex-col gap-3">
          ...
          {error && <p className="text-xs text-red-400">{error}</p>}
          <div className="flex gap-2">
            <button onClick={() => setStage('fillform')} className="flex-1 px-4 py-2.5 rounded-xl border border-[var(--border-color)] text-t-secondary text-sm">Back</button>
            <button onClick={handleSubmit}
              className={`flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold text-white ${op === 'delete' ? 'bg-red-500 hover:bg-red-600' : 'btn-primary'}`}>
              Confirm {op}
            </button>
...
const MobileNav: React.FC<Props> = ({ activePage, onNavigate }) => {
  ...
  const navItems = [
    ...
    { id: 'test-report',   label: 'Report',      icon: <ClipboardList size={20} /> },
    ...(isAdmin ? [
      { id: 'users',       label: 'Users',       icon: <Users size={20} /> },
      { id: 'audit-log',   label: 'Audit Log',   icon: <ScrollText size={20} /> },
    ] : []),
  ];
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-0 inset-x-0 z-50 md:hidden bg-bg-nav border-t border-[var(--border-color)] flex items-center justify-around px-2 h-16">
        {navItems.map(item => (
          <button key={item.id} onClick={() => onNavigate(item.id)}
            className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-colors text-[10px] font-semibold
              ${activePage === item.id ? 'text-c-brand' : 'text-t-muted hover:text-t-primary'}`}>
        ...
        {/* Module shortcut */}
        {modules.length > 0 && (
          <button onClick={() => onNavigate('module', modules[0].name)}
            className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-colors text-[10px] font-semibold
              ${activePage === 'module' ? 'text-c-brand' : 'text-t-muted hover:text-t-primary'}`}>
            <FolderOpen size={20} />Module
          </button>
        )}
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-60 md:hidden" onClick={() => setMenuOpen(false)}>
          <div className="absolute bottom-16 right-0 w-56 bg-bg-surface border border-[var(--border-color)] rounded-2xl shadow-2xl p-2 m-2"
            ...
            {/* Theme editor */}
            <button onClick={() => { setModal('theme'); setMenuOpen(false); }}
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-t-secondary hover:bg-bg-card hover:text-t-primary transition-colors">
            ...
            {isAdmin && (
              <>
                ...
                <p className="text-[10px] font-bold text-t-muted uppercase tracking-wider px-3 py-1">Data</p>

                <button onClick={() => { setModal('export'); setMenuOpen(false); }}
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-t-secondary hover:bg-bg-card hover:text-t-primary transition-colors">
                  <Download size={16} />Export all data
                </button>
                <button onClick={() => { setModal('modules'); setMenuOpen(false); }}
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-t-secondary hover:bg-bg-card hover:text-t-primary transition-colors">
                  <Package size={16} />Manage modules
                </button>
                <button onClick={() => { setModal('tests'); setMenuOpen(false); }}
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-t-secondary hover:bg-bg-card hover:text-t-primary transition-colors">
                  <FlaskConical size={16} />Manage tests
                </button>
                <button onClick={() => { setModal('steps-csv'); setMenuOpen(false); }}
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-t-secondary hover:bg-bg-card hover:text-t-primary transition-colors">
                  <Upload size={16} />Import steps (CSV)
                </button>
                <button onClick={() => { setModal('steps-manual'); setMenuOpen(false); }}
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-t-secondary hover:bg-bg-card hover:text-t-primary transition-colors">
                  <Hash size={16} />Manage steps
                </button>
              </>
            )}
      ...
      {activeModal === 'modules'      && <ImportModulesModal    onClose={close} onDone={close} />}
      {activeModal === 'tests'        && <ImportTestsModal      onClose={close} onDone={close} />}
      {activeModal === 'steps-csv'    && <ImportStepsModal      onClose={close} onDone={close} />}
      {activeModal === 'steps-manual' && <ImportStepsManualModal onClose={close} onDone={close} />}
      {activeModal === 'theme'        && (
