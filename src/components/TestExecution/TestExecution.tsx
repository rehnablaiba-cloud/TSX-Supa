...
const TestExecution: React.FC<Props> = ({
  ...
  const load = useCallback(async () => {
    try {
      const { step_results, module_tests: mts } = await fetchTestExecution(
        currentMtId
      );
      ...
      const allPaths = mapped.flatMap((s) => [
        ...s.action_image_urls,
        ...
        setSteps((prev) =>
          prev.map((s) =>
            s.stepId === step.stepId
              ? { ...s, status, remarks: remarks ?? s.remarks }
              : s
          )
        );
  ...
  const handleUndoAll = useCallback(async () => {
    ...
    setSaving("all");
    try {
      await resetAllstep_results(module_name);
      setSteps((prev) =>
        prev.map((s) => ({ ...s, status: "pending", remarks: "" }))
      );
  ...
  const doneSteps = useMemo(
    () => steps.filter((s) => !s.is_divider && s.status !== "pending"),
    [steps]
  );
  const totalSteps = useMemo(() => steps.filter((s) => !s.is_divider), [steps]);
  const passCount = useMemo(
    () => steps.filter((s) => !s.is_divider && s.status === "pass").length,
    [steps]
  );
  const failCount = useMemo(
    () => steps.filter((s) => !s.is_divider && s.status === "fail").length,
    [steps]
  );
  const pendingCount = useMemo(
    () => steps.filter((s) => !s.is_divider && s.status === "pending").length,
    [steps]
  );
  const progressPct =
    totalSteps.length > 0
      ? Math.round((doneSteps.length / totalSteps.length) * 100)
      : 0;
  ...
  const filteredSteps = useMemo(
    () =>
      steps.filter((s) => {
        ...
        if (search && !s.action.toLowerCase().includes(search.toLowerCase()))
          return false;
        return true;
      }),
    [steps, filter, search]
  ...
  const test_name =
    module_tests.find((mt) => mt.id === currentMtId)?.test?.name ?? testsName;

  const exportData: FlatData[] = useMemo(
    () =>
      steps
    ...
    <div className="flex-1 flex flex-col">
          ...
          {passCount > 0 && (
            <div
              ...
              style={{
                width: `${
                  totalSteps.length > 0
                    ? (passCount / totalSteps.length) * 100
                    : 0
                }%`,
              }}
            />
          ...
          {failCount > 0 && (
            <div
              ...
              style={{
                width: `${
                  totalSteps.length > 0
                    ? (failCount / totalSteps.length) * 100
                    : 0
                }%`,
              }}
            />
          ...
          <button
            ...
            className={`text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide transition-colors ${
              filter === f
                ...
                : "bg-bg-card text-t-muted hover:text-t-primary"
            }`}
          >
            {f}
          </button>
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-28 md:pb-6 flex flex-col gap-2 pt-2">
        {filteredSteps.length === 0 && (
          <p className="text-sm text-t-muted text-center py-12">
        ...
        {filteredSteps.map((step) => {
          if (step.is_divider) {
            ...
            return (
              ...
              >
                <span
                  className={`rounded-full shrink-0 ${cfg.dotClass}`}
                  style={{ width: cfg.dotSize, height: cfg.dotSize }}
                />
                <span className={`${cfg.textClass} ${cfg.fontSize}`}>
                  {step.action}
                </span>
              </div>
            );
          ...
          const actionSigned = step.action_image_urls
            .map((p) => signedImages[p])
            .filter(Boolean);
          const expectedSigned = step.expected_image_urls
            .map((p) => signedImages[p])
            .filter(Boolean);
            ...
            >
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-t-primary">
                    {step.action || (
                      <span className="italic text-t-muted">
                        No action text
                      </span>
                    )}
                  </p>
              ...
              {(isExpanded || step.status === "fail") && (
                <textarea
                  ...
                  onBlur={(e) => {
                    if (e.target.value !== step.remarks)
                      handleSetStatus(step, step.status, e.target.value);
                  }}
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {step.status !== "pending" && (
                  <button
                    onClick={() => handleSetStatus(step, "pending")}
                    disabled={isSaving}
          ...
          options={[
            {
              ...
              onConfirm: () =>
                exportExecutionCSV(module_name, test_name, exportData),
            },
            {
              label: "PDF",
              ...
              onConfirm: () =>
                exportExecutionPDF(module_name, test_name, exportData),
            },
          ]}
        />
      )}
